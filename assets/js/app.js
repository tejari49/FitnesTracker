...
function inferPose(item) {
  ...
  if (["squat", "beinpresse", "leg_press", "lunges", "ausfallschritt"].some((t) => key.includes(t))) return "squat";
  if (["deadlift", "kreuzheben", "rudern", "row", "pull_ups", "klimm"].some((t) => key.includes(t))) return "hinge";
  if (["push_ups", "liegest", "bankdr", "chest_press", "dips"].some((t) => key.includes(t))) return "push";
  if (["shoulder_press", "schulter", "trizeps", "curl", "dumbbell", "kurzhantel"].some((t) => key.includes(t))) return "press";
  if (["kettlebell", "swing"].some((t) => key.includes(t))) return "kettlebell";
  return "barbell";
}
...
class App {
  ...
  renderPersonAvatar(variant = "female", size = "default", pose = "barbell") {
    ...
    const equipmentByPose = {
      barbell: `<g opacity="0.98"><line x1="28" y1="100" x2="152" y2="100" stroke="#7c8b9b" stroke-width="7" stroke-linecap="round"/><rect x="10" y="89" width="16" height="22" rx="4" fill="#94a3b8"/><rect x="154" y="89" width="16" height="22" rx="4" fill="#94a3b8"/><rect x="2" y="85" width="8" height="30" rx="3" fill="#475569"/><rect x="170" y="85" width="8" height="30" rx="3" fill="#475569"/></g>`,
      press: `<g opacity="0.95"><rect x="28" y="53" width="124" height="7" rx="3" fill="#7c8b9b"/><rect x="20" y="47" width="8" height="18" rx="3" fill="#94a3b8"/><rect x="152" y="47" width="8" height="18" rx="3" fill="#94a3b8"/></g>`,
      hinge: `<g opacity="0.9"><line x1="34" y1="132" x2="146" y2="132" stroke="#93a4b5" stroke-width="7" stroke-linecap="round"/><circle cx="28" cy="132" r="9" fill="#64748b"/><circle cx="152" cy="132" r="9" fill="#64748b"/></g>`,
      squat: `<g opacity="0.9"><rect x="36" y="122" width="108" height="10" rx="5" fill="#8da2b7"/><rect x="44" y="132" width="10" height="18" rx="5" fill="#64748b"/><rect x="126" y="132" width="10" height="18" rx="5" fill="#64748b"/></g>`,
      push: `<g opacity="0.35"><rect x="20" y="170" width="140" height="18" rx="9" fill="#4b5d72"/></g>`,
      cardio: `<g opacity="0.5"><ellipse cx="134" cy="162" rx="22" ry="8" fill="#445569"/><circle cx="134" cy="159" r="14" fill="none" stroke="#93a4b5" stroke-width="3"/><rect x="120" y="88" width="28" height="64" rx="8" fill="#5a6c80"/><rect x="112" y="104" width="44" height="7" rx="3" fill="#cbd5e1"/></g>`,
      rope: `<g opacity="0.6"><path d="M18 145 C36 118, 62 168, 84 138 S124 116, 162 145" fill="none" stroke="#d3deea" stroke-width="5.5" stroke-linecap="round"/></g>`,
      core: `<g opacity="0.5"><rect x="22" y="170" width="136" height="16" rx="8" fill="#516173"/></g>`,
      kettlebell: `<g opacity="0.85"><path d="M132 138 C144 138,154 147,154 160 C154 176,142 188,126 188 C110 188,98 176,98 160 C98 147,108 138,120 138 Z" fill="#5f7387"/><path d="M116 136 C116 123,121 116,131 116 C141 116,146 123,146 136" fill="none" stroke="#d7e4ef" stroke-width="5" stroke-linecap="round"/></g>`
    };
    const limbPose = {
      barbell: { leftArm: "M70 108 C58 120,53 130,48 141", rightArm: "M120 108 C132 120,137 130,142 141", leftLeg: "M84 181 C82 195,78 203,74 212", rightLeg: "M106 181 C108 194,112 203,116 212" },
      squat: { leftArm: "M68 114 C58 116,48 117,42 115", rightArm: "M122 114 C132 116,142 117,148 115", leftLeg: "M88 181 C78 190,70 196,62 200", rightLeg: "M102 181 C112 190,120 196,128 200" },
      hinge: { leftArm: "M72 110 C62 124,56 132,50 140", rightArm: "M118 110 C128 124,134 132,140 140", leftLeg: "M84 181 C78 196,74 205,72 214", rightLeg: "M106 181 C112 196,116 205,118 214" },
      push: { leftArm: "M68 112 C55 118,46 126,40 136", rightArm: "M122 112 C135 118,144 126,150 136", leftLeg: "M86 181 C82 194,74 202,66 208", rightLeg: "M104 181 C108 194,116 202,124 208" },
      press: { leftArm: "M72 106 C64 92,60 78,58 62", rightArm: "M118 106 C126 92,130 78,132 62", leftLeg: "M84 181 C80 194,76 203,72 212", rightLeg: "M106 181 C110 194,114 203,118 212" },
      cardio: { leftArm: "M72 110 C64 120,56 125,52 130", rightArm: "M118 110 C126 120,134 125,138 130", leftLeg: "M84 181 C74 192,65 197,58 201", rightLeg: "M106 181 C114 193,124 200,132 207" },
      rope: { leftArm: "M72 112 C58 126,42 137,30 145", rightArm: "M118 112 C132 126,148 137,160 145", leftLeg: "M84 181 C82 196,78 205,75 214", rightLeg: "M106 181 C108 196,112 205,115 214" },
      core: { leftArm: "M74 116 C66 124,60 132,54 140", rightArm: "M116 116 C124 124,130 132,136 140", leftLeg: "M84 181 C74 183,63 184,50 186", rightLeg: "M106 181 C116 183,127 184,140 186" },
      kettlebell: { leftArm: "M72 108 C62 122,56 134,54 148", rightArm: "M118 108 C128 122,134 134,136 148", leftLeg: "M84 181 C80 194,74 202,66 208", rightLeg: "M106 181 C110 194,116 202,124 208" }
    };
    const selectedPose = equipmentByPose[pose] ? pose : "barbell";
    const activeLimbPose = limbPose[selectedPose] || limbPose.barbell;
    const ponytail = safeVariant === "female" ? `<path d="M124 64 C148 74,150 100,132 112 C125 99,114 87,100 81" fill="url(#hairGrad-${safeVariant}-${safeSize})"/>` : "";
    return `<div class="toy-avatar ${sizeClass}" aria-hidden="true"><svg viewBox="0 0 180 220" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="bgGrad-${safeVariant}-${safeSize}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${palette.bubble}" stop-opacity="0.95"/><stop offset="100%" stop-color="#1e293b" stop-opacity="0.15"/></linearGradient><linearGradient id="hairGrad-${safeVariant}-${safeSize}" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="${palette.hair2}"/><stop offset="100%" stop-color="${palette.hair}"/></linearGradient><linearGradient id="topGrad-${safeVariant}-${safeSize}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${palette.top}"/><stop offset="100%" stop-color="${palette.top2}"/></linearGradient><linearGradient id="shortGrad-${safeVariant}-${safeSize}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${palette.shorts}"/><stop offset="100%" stop-color="${palette.shorts2}"/></linearGradient><radialGradient id="spot-${safeVariant}-${safeSize}" cx="50%" cy="20%" r="80%"><stop offset="0%" stop-color="#ffffff" stop-opacity="0.25"/><stop offset="100%" stop-color="#ffffff" stop-opacity="0"/></radialGradient></defs><rect x="0" y="0" width="180" height="220" fill="url(#bgGrad-${safeVariant}-${safeSize})"/><rect x="0" y="0" width="180" height="220" fill="url(#spot-${safeVariant}-${safeSize})"/><ellipse cx="92" cy="199" rx="56" ry="11" fill="#0f172a" opacity="0.24"/><g><ellipse cx="94" cy="68" rx="41" ry="39" fill="${palette.skin}"/><path d="M56 61 C60 30,80 20,102 24 C124 28,133 45,130 67 C121 53,109 46,93 46 C77 46,66 50,56 61 Z" fill="url(#hairGrad-${safeVariant}-${safeSize})"/>${ponytail}<circle cx="79" cy="70" r="11" fill="#fff"/><circle cx="107" cy="70" r="11" fill="#fff"/><circle cx="80" cy="72" r="6.3" fill="#3f2d2a"/><circle cx="108" cy="72" r="6.3" fill="#3f2d2a"/><circle cx="83" cy="69" r="2" fill="#fff"/><circle cx="111" cy="69" r="2" fill="#fff"/><path d="M71 58 C76 54,83 54,89 58" fill="none" stroke="#412b24" stroke-width="3.2" stroke-linecap="round"/><path d="M98 58 C103 54,110 54,116 58" fill="none" stroke="#412b24" stroke-width="3.2" stroke-linecap="round"/><path d="M82 90 C88 96,100 96,108 90" fill="none" stroke="#a05652" stroke-width="3" stroke-linecap="round"/><path d="${activeLimbPose.leftArm}" fill="none" stroke="${palette.skin}" stroke-width="13" stroke-linecap="round"/><path d="${activeLimbPose.rightArm}" fill="none" stroke="${palette.skin}" stroke-width="13" stroke-linecap="round"/><rect x="66" y="102" width="58" height="56" rx="18" fill="url(#topGrad-${safeVariant}-${safeSize})"/><rect x="74" y="150" width="44" height="36" rx="12" fill="url(#shortGrad-${safeVariant}-${safeSize})"/><path d="${activeLimbPose.leftLeg}" fill="none" stroke="${palette.skin}" stroke-width="13" stroke-linecap="round"/><path d="${activeLimbPose.rightLeg}" fill="none" stroke="${palette.skin}" stroke-width="13" stroke-linecap="round"/><ellipse cx="74" cy="212" rx="12" ry="5" fill="#6b7280"/><ellipse cx="116" cy="212" rx="12" ry="5" fill="#6b7280"/></g>${equipmentByPose[selectedPose] || equipmentByPose.barbell}</svg></div>`;
  }
