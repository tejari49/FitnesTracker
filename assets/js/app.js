...
class App {
  ...
  renderPersonAvatar(variant = "female", size = "default", pose = "barbell") {
    ...
    return `<div class="toy-avatar ${sizeClass}" aria-hidden="true"><svg viewBox="0 0 180 220" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="bgGrad-${safeVariant}-${safeSize}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${palette.bubble}" stop-opacity="0.95"/><stop offset="100%" stop-color="#1e293b" stop-opacity="0.15"/></linearGradient><linearGradient id="hairGrad-${safeVariant}-${safeSize}" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="${palette.hair2}"/><stop offset="100%" stop-color="${palette.hair}"/></linearGradient><linearGradient id="topGrad-${safeVariant}-${safeSize}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${palette.top}"/><stop offset="100%" stop-color="${palette.top2}"/></linearGradient><linearGradient id="shortGrad-${safeVariant}-${safeSize}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${palette.shorts}"/><stop offset="100%" stop-color="${palette.shorts2}"/></linearGradient></defs><rect x="0" y="0" width="180" height="220" fill="url(#bgGrad-${safeVariant}-${safeSize})"/><g><path d="M62 50 C64 28, 83 18, 101 20 C121 22,133 38,130 56 C124 68,111 75,95 75 C74 75,61 66,62 50 Z" fill="url(#hairGrad-${safeVariant}-${safeSize})"/>${ponytail}<circle cx="94" cy="62" r="36" fill="${palette.skin}"/><circle cx="80" cy="62" r="12" fill="#fff"/><circle cx="108" cy="62" r="12" fill="#fff"/><circle cx="82" cy="64" r="7" fill="#4a362e"/><circle cx="110" cy="64" r="7" fill="#4a362e"/><path d="${activeLimbPose.leftArm}" fill="none" stroke="${palette.skin}" stroke-width="14" stroke-linecap="round"/><path d="${activeLimbPose.rightArm}" fill="none" stroke="${palette.skin}" stroke-width="14" stroke-linecap="round"/><path d="M66 104 C70 96,77 94,93 94 C111 94,120 97,124 104 L121 145 C120 152,114 158,106 158 H81 C73 158,67 152,66 145 Z" fill="url(#topGrad-${safeVariant}-${safeSize})"/><path d="M74 146 H117 C118 157,116 170,113 182 H78 C75 171,73 158,74 146 Z" fill="url(#shortGrad-${safeVariant}-${safeSize})"/><path d="${activeLimbPose.leftLeg}" fill="none" stroke="${palette.skin}" stroke-width="14" stroke-linecap="round"/><path d="${activeLimbPose.rightLeg}" fill="none" stroke="${palette.skin}" stroke-width="14" stroke-linecap="round"/></g>${equipmentByPose[selectedPose]}</svg></div>`;
  }
